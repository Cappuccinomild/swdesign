...
$db->query = "SELECT COUNT(GoodsID) FROM feedback
							WHERE CustomerID = '".$id."' AND permit = 1 AND NOT GoodsID = 0 ";
...
if($db->result){//값이 존재할 경우
	 ...
	 for($board_num = 1 ; $board_num <= $MAX/16 ; $board_num = $board_num+1){
			 $base->content .="<a href = './customer_ordered.php?page=".$board_num."'> ".$board_num. " ";
	 }
	 if($page < $MAX/16){
		 $board_num = $page + 1;
		 $base->content .="<a href = './customer_ordered.php?page=".$board_num."'> 다음 ";
	 }
}
